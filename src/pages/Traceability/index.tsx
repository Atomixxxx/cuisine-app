...
export default function Traceability() {
  ...
  const markProductAsUsed = useAppStore(s => s.markProductAsUsed);
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((p) => {
      if (!showUsed && p.status === 'used') return false;
      if (searchText) {
  ...
  }, [products, searchText, filterCategory, filterSupplier, dateFrom, dateTo, showUsed]);

  const urgentProducts = useMemo(
    () =>
      filteredProducts.filter((product) => {
        if (product.status === 'used') return false;
  ...
  const handleMarkAsUsed = useCallback(
    ...
    [markProductAsUsed, loadInitialProducts]
  );
        ...
        {activeTab === 'history' && (
          <div className="flex flex-col gap-3 py-2">
            ...
            <div className="flex flex-col gap-2 p-3 rounded-2xl glass-card glass-panel">
              ...
              <div className="flex items-center justify-between rounded-xl app-surface-2 px-3 py-2.5">
                <div>
                  <p className="ios-body app-text font-medium">Afficher les produits utilises</p>
                  <p className="ios-caption app-muted">Historique de consommation</p>
                </div>
                <button
                  type="button"
                  aria-label="Afficher les produits utilises"
                  aria-pressed={showUsed}
                  onClick={() => setShowUsed((value) => !value)}
                  className={cn(
                    'relative inline-flex h-7 w-12 items-center rounded-full transition-colors',
                    showUsed ? 'bg-[color:var(--app-accent)]' : 'bg-[color:var(--app-surface-3)]',
                  )}
                >
                  <span
                    className={cn(
                      'inline-block h-5 w-5 transform rounded-full bg-white transition-transform',
                      showUsed ? 'translate-x-6' : 'translate-x-1',
                    )}
                  />
                </button>
              </div>
      ...
      {products.map((product) => {
        ...
        return (
          <div
            ...
            className={cn(
              'flex items-center gap-3 p-3.5 rounded-2xl app-card cursor-pointer active:opacity-70 transition-opacity',
              isUsed && 'opacity-50',
            )}
          >
            {/* Icon */}
            ...
            <div className="flex flex-col items-end gap-1 flex-shrink-0">
              ...
              {isUsed ? (
                <span className="px-2 py-0.5 text-[12px] font-medium rounded-full bg-[color:var(--app-surface-3)] app-muted">
                  Utilise
                </span>
              ) : (
